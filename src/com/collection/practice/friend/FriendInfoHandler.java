...
//				friends[i].showData();
	...
	public void showBasicData() {
		...
		if (!friends.isEmpty()) { // 값이 없으면 true 반환
			for (int i = 0; i < friends.size(); i++) {
...
//				friends[i].showBasicInfo();
	...
	public void deleteData() {
		...
		for (int i = 0; i < friends.size(); i++) { // 배열의 갯수만큼 반복
			if (friends.get(i).name.equals(name)) { // 입력한 값과 배열의 값이 같은 경우
...
//					temp = friends.get(i); // 해당 배열을 변수에 temp에 대입
//					for (int j = i; j <= friends.size(); j++) { // 배열의 갯수만큼 반복
